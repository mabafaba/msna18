...
expand_composite_indicators_to_set_intersections(testdata,c("pin1","pin2","pin3")) %>% head
...
testdata<-data.frame(test=sample(letters[1:5],100,T),pin1=sample(c(0,1),100,T),pin2=sample(c(0,1),100,T),pin3=sample(c(0,1),100,T))

# which ones should be intersected?
composite_indicator_names<-c("pin1","pin2","pin3")

# assuming they are coercible to logical (e.g. 0's and  1's)
# you can create TRUE/FALSE columns for call combinations with this:
intersected_composites<- expand_composite_indicators_to_set_intersections(testdata,compnames)
