...
data_no_sm <- data[!(names(data) %in% sm_variables)] %>% to_alphanumeric_lowercase_colnames_df
      ...
      varasnames<-data_no_sm[[var]]
        for(i in ch){
          ...
          if(!(i %in% (choices_questionnaire))){
            print(i)
            dd <- match(i, questionnaire$choices_per_variable[[var]]$label)
            print(dd)
            to = match(i, questionnaire$choices_per_variable[[var]]$label) %>% unlist
...
named <- as.data.frame(named, stringsAsFactors = F)
names(named) <-names(data_no_sm)
myset <- as.data.frame(myset, stringsAsFactors = F)
names(myset) <- sm_variables


alltogether <- cbind(named,myset)
