...
# 
# 
# contains_multiple_conditions<-function(x){
#   x<-strsplit(x,"[[:space:]]") %>% unlist %>% strsplit("\\(\\)") %>% unlist
#   operators<-sapply(x,identify_operator_in_string)
#   any(!is.na(operators))
# }
# 
# string_to_hierarchical_condition<-function(condition){
#   hierarchical_condition_string<-string_w_brackets_to_hierarchical_list(condition,only_multiple_conditions=T)
#   hierarchical_condition_string<-reduce_single_item_lists(hierarchical_condition_string)
#   
# }
# # con<-hierarchical_condition_string[[3]]
# is_incomplete_condition_element<-function(x){
# con<-x
# recognised<-any(is.list(con),is_select_one_condition(con),
# is_select_multiple_condition(con),
# is_numeric_condition(con),
# con %in% c("!","&","|")
# )
# return(!recognised)
# }
# 
# # reattach_unidentified_strings<-function(l){
# #   # recursion
# #   for(i in 1:length(l)){
# #     if(is.list(l[[i]])){l[[i]]<-reattach_unidentified_strings(l[[i]])}
# #   }
# #   # recombine strings
# #   for(i in length(l):2){
# #   # if two adjacent items are both not lists, and any of them an incomplete condition item, and neither an and/or/not operator..
# #     # then concatenate the two
# #   if(!is.list(l[[i]]) & !is.list(l[[i-1]]) & any(is_incomplete_condition_element(l[[i]]),is_incomplete_condition_element(l[[i-1]]))){
# #     print(l)
# #     l[[i-1]]<-paste0(l[[i-1]],l[[i]])
# #     l[[i]]<-NULL
# #   }
# # }
# #   l
...
# # has unit tests
# string_w_brackets_to_hierarchical_list<-function(x,only_multiple_conditions=T){
#   # if we don't want to take apart brackets inside numeric conditions..
#   if(only_multiple_conditions){
#     # .. then check if it is a numeric condition, and if yes, return the input as is
#     if(!contains_multiple_conditions(x)){return(x)}
#     }
