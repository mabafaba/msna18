...
#' Map to Analysis plan
#'
#' creates a `analysisplan` dataframe from the data
#'
#' @param data
#' @param analysis_plan_user analysis plan filled by user in the Excel input file
#' @details Creates a `analysisplan` dataframe from the data and the analysis plan. It selects which hypothesis type
#' to be run for each analysis line.
#' @return a `analysisplan` dataframe
#' @examples map_to_analysisplan_custom_user_plan(data, analysis_plan_user)
#' @export
...
map_to_analysisplan_custom_user_plan <- function(data, analysis_plan_user) {
  ...
  if(nrow(analysis_plan_user_ALL) > 0) {
    ...
    analysis_plan_user_ALL <- all_dependents %>% lapply(function(dep) {
      ...
      this_analysis_plan_user_ALL
      }
      )%>% do.call(rbind,.)
  } 
  
  analysis_plan_user <- rbind(analysis_plan_user[analysis_plan_user$variable!="..all..", ], analysis_plan_user_ALL) %>% 
    unique
  
  analysis_plan_user <- analysis_plan_user[!apply(analysis_plan_user, 1, 
                                                  function(x) { all(is.na(x)|grepl("^[[:space:]]*$",x)) }), ]
  ...
  if(any(has_no_var)){analysis_plan_user <- analysis_plan_user[!has_no_var, ]
                      .write_to_log("Some rows in the user defined analysisplan are not completely empty, 
  ...
  if(any(!var.exists.in.data)) {
    stop(paste0("analysis plan input contains variable names that could not be found in the data or composite indicators:\n",
