...
#     if(case!="CASE_group_difference_categorical_categorical"){
#       p<-p+geom_text(aes(label=paste(tryround(datag$value,2)," (",tryround(datag$cl_lw,2)," , ",tryround(datag$cl_up,2),")",sep="")), family=font_family,size = fsize,color="#42423E")
#     }else{
#       p<-p+geom_text(aes(label=paste(tryround(datag$value,1),"% (",tryround(datag$cl_lw,1)," , ",tryround(datag$cl_up,1),")",sep="")), family=font_family,size = fsize,color="#42423E")
#     }
# 
...
#                     axis.text.y=element_text(family=font_family,size=tryround(fsize*3.4,0),color="#42423E"))
#     
#     p.top   <-  p.bot + theme(
#       axis.text.x = element_text(family=font_family,size=tryround(fsize*3.4,0)),
#       axis.text.y = element_text(family=font_family,size=tryround(fsize*3.4,0),color="white")
...
# terrible loop
results$analysisplan_log$include_in_html<-"no"
results$analysisplan_log$include_in_html[1:50]<-"yes"
whichtoreport<-which(results$analysisplan_log$include_in_html=="yes")
if(length(whichtoreport)>50){
  whichtoplot<-whichtoreport[1:50]
}
for (mycount in whichtoreport){
  ...
  if(results$results[[mycount]]$message=="success (or unidentified issue)"){
    ...
    if(case=="CASE_group_difference_numerical_categorical" & !is.null(results$results[[mycount]]$hypothesis.test$name)){
      ...
      reportcharthtml<-paste0("<img src='",reportchartpath,"' width=50%>")
    cat('\n\n\n## Graph regular\n\n')
