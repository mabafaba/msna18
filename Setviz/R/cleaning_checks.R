...
test_that("confidence_intervals_mean inputs correct",{
  ...
  expect_is(confidence_intervals_mean(tf$numeric[1], design = design), "data.frame") #numerical var
  expect_is(confidence_intervals_mean(tf$numeric_NA_heavy[1], design = design), "data.frame") #numerical var
  expect_is(confidence_intervals_mean(tf$logical[1], design = design), "data.frame")
  expect_warning(confidence_intervals_mean(tf$numeric[1], tf$select_one[2] , design = design))
  expect_error(confidence_intervals_mean(tf$select_one[1], design = design))
  expect_error(confidence_intervals_mean(tf$select_one[1], design = design))
  expect_error(confidence_intervals_mean(tf$NAs[1], design = design))
  expect_error(confidence_intervals_mean(tf$fake[1], design = design)) #nonexistent.var
  expect_error(confidence_intervals_mean(tf$select_multiple[1], design = design)) # select multiple
})
